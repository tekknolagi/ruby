...
gc_page_sweep(rb_objspace_t *objspace, rb_heap_t *heap, struct heap_page *sweep_page)
{
    ...
    if (0)
      fprintf(stderr, "gc_page_sweep(%d): freed?: %d, limt: %d, freed_slots: %d, empty_slots: %d, final_slots: %d\n",
	      (int)rb_gc_count(),
...
#define SET(name, attr) rb_hash_aset(hash, sym_##name, SIZET2NUM(attr))
    ...
    SET(heap_eden_page_length, heap_eden->used);
    SET(heap_tomb_page_length, heap_tomb->used);
