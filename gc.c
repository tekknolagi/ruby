...
{
    unsigned int bin = rfree_size_bin_index(slots);
...
gc_compact_heap(rb_objspace_t *objspace, page_compare_func_t *comparator, struct RMoved * moved_list)
{
    ...
    while (not_met(&free_cursor, &scan_cursor)) {
        ...
        while ((is_payload_object((VALUE)free_cursor.slot) || BUILTIN_TYPE((VALUE)free_cursor.slot) != T_NONE) && not_met(&free_cursor, &scan_cursor)) {
            /* Re-poison slot if it's not the one we want */
            if (free_slot_poison) {
        ...
        while ((is_payload_object((VALUE)scan_cursor.slot) || !gc_is_moveable_obj(objspace, (VALUE)scan_cursor.slot)) && not_met(&free_cursor, &scan_cursor)) {
