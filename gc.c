...
{
    int bin = rfree_size_bin_index(size);
...
heap_page_add_freeobj(rb_objspace_t *objspace, struct heap_page *page, VALUE obj)
{
    ...
    int is_left_none = RVALUE_SAME_PAGE_P(obj, left) && !is_payload_slot(left) && BUILTIN_TYPE(left) == T_NONE;
    int is_right_none = RVALUE_SAME_PAGE_P(obj, right) && !is_payload_slot(right) && BUILTIN_TYPE(right) == T_NONE;
...
rb_free_payload(VALUE payload)
{
    ...
    VALUE payload_start = payload - sizeof(struct RPayload);

    int length = RANY(payload_start)->as.payload.length;
    GC_ASSERT(length > 0);

    struct heap_page *page = GET_HEAP_PAGE(payload_start);

    for (int i = 0; i < length; i++) {
        VALUE p = payload_start + i * sizeof(RVALUE);
...
{
    memset((void *)obj, 0, length * sizeof(RVALUE));
    ...
    MEMCPY((void *)obj, &buf, RVALUE, 1);

    objspace->payload_slots += length;

#if __has_feature(address_sanitizer)
    for (int i = 0; i < length; i++) {
        asan_poison_object(obj + i * sizeof(RVALUE));
    }
#endif
...
static VALUE
newobj_allocate(size_t size, VALUE *payload)
{
    rb_objspace_t *objspace = &rb_objspace;
    VALUE obj;

    unsigned int slots = (unsigned int)CEILDIV((size + sizeof(struct RPayload)), sizeof(RVALUE));
    ...
    RB_VM_LOCK_ENTER();
    {
        ...
        if (!(during_gc ||
            ...
            // TODO: this is (mostly) a copy of newobj_slowpath, refactor please
            if (UNLIKELY(during_gc || ruby_gc_stressful)) {
                if (during_gc) {
                    dont_gc_on();
                    during_gc = 0;
    ...
    newobj_init_payload(objspace, obj + sizeof(RVALUE), slots - 1);

    // TODO: don't assume that payload is not a NULL pointer
    // TODO: don't assume that slots > 1
    (*payload) = obj + sizeof(RVALUE) + sizeof(struct RPayload);

    return obj;
}

static inline VALUE
newobj_of(VALUE klass, VALUE flags, VALUE v1, VALUE v2, VALUE v3, int wb_protected, unsigned int slots)
{
    rb_objspace_t *objspace = &rb_objspace;
    VALUE obj;
...
{
    GC_ASSERT((flags & FL_WB_PROTECTED) == 0);
    return newobj_of(klass, flags, 0, 0, 0, FALSE, 1);
}
...
{
    GC_ASSERT((flags & FL_WB_PROTECTED) == 0);
    return newobj_of(klass, flags, 0, 0, 0, TRUE, 1);
}
...
rb_newobj(void)
{
    return newobj_of(0, T_NONE, 0, 0, 0, FALSE, 1);
}

VALUE
rb_newobj_of(VALUE klass, VALUE flags)
{
    return newobj_of(klass, flags & ~FL_WB_PROTECTED, 0, 0, 0, flags & FL_WB_PROTECTED, 1);
}
...
rb_newobj_with(VALUE src)
{
    ...
    return newobj_of(klass, flags & ~FL_WB_PROTECTED, v1, v2, v3, flags & FL_WB_PROTECTED, 1);
}
...
// TODO: refactor this
VALUE
rb_imemo_iseq_new(enum imemo_type type, VALUE v1, VALUE v2, VALUE v3, VALUE v0, VALUE *payload)
{
    VALUE flags = T_IMEMO | (type << FL_USHIFT);
    // return newobj_of(v0, flags, v1, v2, v3, TRUE, 1 + rfree_size_bin_index(sizeof(struct rb_iseq_constant_body)));
    VALUE obj = newobj_allocate(sizeof(RVALUE) + sizeof(struct rb_iseq_constant_body), payload);
    newobj_init(v0, flags, v1, v2, v3, TRUE, &rb_objspace, obj);
    return obj;
}
...
{
    VALUE flags = T_IMEMO | (type << FL_USHIFT);
    return newobj_of(v0, flags, v1, v2, v3, TRUE, 1);
}
...
rb_imemo_tmpbuf_new(VALUE v1, VALUE v2, VALUE v3, VALUE v0)
{
    ...
    return newobj_of(v0, flags, v1, v2, v3, FALSE, 1);
}
...
rb_class_allocate_instance(VALUE klass)
{
    ...
    return newobj_of(klass, flags, Qundef, Qundef, Qundef, RGENGC_WB_PROTECTED_OBJECT, 1);
}
...
rb_data_object_wrap(VALUE klass, void *datap, RUBY_DATA_FUNC dmark, RUBY_DATA_FUNC dfree)
{
    ...
    return newobj_of(klass, T_DATA, (VALUE)dmark, (VALUE)dfree, (VALUE)datap, FALSE, 1);
}
...
rb_data_typed_object_wrap(VALUE klass, void *datap, const rb_data_type_t *type)
{
    ...
    if (klass) Check_Type(klass, T_CLASS);
    return newobj_of(klass, T_DATA, (VALUE)type, (VALUE)1, (VALUE)datap, type->flags & RUBY_FL_WB_PROTECTED, 1);
}
...
gc_page_sweep(rb_objspace_t *objspace, rb_heap_t *heap, struct heap_page *sweep_page)
{
	...
	if (bitset) {
	    ...
	    do {
                VALUE vp = (VALUE)p;
                GC_ASSERT((unsigned short)NUM_IN_PAGE(vp) < sweep_page->total_slots);
                asan_unpoison_object(vp, false);
		if (bitset & 1) {
...
gc_compact_heap(rb_objspace_t *objspace, page_compare_func_t *comparator, struct RMoved * moved_list)
{
    ...
    while (not_met(&free_cursor, &scan_cursor)) {
        ...
        while ((is_payload_slot((VALUE)free_cursor.slot) || BUILTIN_TYPE((VALUE)free_cursor.slot) != T_NONE) && not_met(&free_cursor, &scan_cursor)) {
            /* Re-poison slot if it's not the one we want */
            if (free_slot_poison) {
        ...
        while ((is_payload_slot((VALUE)scan_cursor.slot) || !gc_is_moveable_obj(objspace, (VALUE)scan_cursor.slot)) && not_met(&free_cursor, &scan_cursor)) {
