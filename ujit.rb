module UJIT
  ...
  if defined?(GEN_STATS)
    at_exit do
      ...
      counters = stat_counters.filter { |key, _| key.start_with?('swb_') }
      counters.transform_keys! { |key| key.to_s.delete_prefix('swb_') }
