...
gen_oswb_cfunc(jitstate_t* jit, ctx_t* ctx, struct rb_call_data * cd, const rb_callable_method_entry_t *cme, int32_t argc)
{
    ...
    {
        GEN_COUNTER_INC(cb, swb_cfunc_ruby_array_varg);
        return false;
    }
    ...
    {
        GEN_COUNTER_INC(cb, swb_cfunc_argc_mismatch);
        return false;
    }

    // Don't JIT functions that need C stack arguments for now
    if (argc + 1 > NUM_C_ARG_REGS) {
        GEN_COUNTER_INC(cb, swb_cfunc_toomany_args);
        return false;
    }
    ...
    {
        uint8_t *receiver_not_heap = COUNTED_EXIT(side_exit, swb_se_receiver_not_heap);
        test(cb, REG0, imm_opnd(RUBY_IMMEDIATE_MASK));
    ...
    jne_ptr(cb, COUNTED_EXIT(side_exit, swb_se_cc_klass_differ));
    ...
    if (cfunc_needs_frame(cfunc))
    {
        ...
        cmp(cb, REG_CFP, REG0);
        jle_ptr(cb, COUNTED_EXIT(side_exit, swb_se_cf_overflow));
...
gen_oswb_iseq(jitstate_t* jit, ctx_t* ctx, struct rb_call_data * cd, const rb_callable_method_entry_t *cme, int32_t argc)
{
    ...
    if (num_params != argc) {
        GEN_COUNTER_INC(cb, swb_iseq_argc_mismatch);
        return false;
    }
    ...
    if (!rb_simple_iseq_p(iseq)) {
        ...
        GEN_COUNTER_INC(cb, swb_iseq_not_simple);
        return false;
    }

    if (vm_ci_flag(cd->ci) & VM_CALL_TAILCALL) {
        // We can't handle tailcalls
        GEN_COUNTER_INC(cb, swb_iseq_tailcall);
        return false;
    }
...
gen_opt_send_without_block(jitstate_t* jit, ctx_t* ctx)
{
    ...
    case VM_METHOD_TYPE_ATTRSET:
        GEN_COUNTER_INC(cb, swb_ivar_set_method);
        return false;
    case VM_METHOD_TYPE_BMETHOD:
        GEN_COUNTER_INC(cb, swb_bmethod);
        return false;
    case VM_METHOD_TYPE_IVAR:
        GEN_COUNTER_INC(cb, swb_ivar_get_method);
        return false;
    case VM_METHOD_TYPE_ZSUPER:
        GEN_COUNTER_INC(cb, swb_zsuper_method);
        return false;
    case VM_METHOD_TYPE_ALIAS:
        GEN_COUNTER_INC(cb, swb_alias_method);
        return false;
    case VM_METHOD_TYPE_UNDEF:
        GEN_COUNTER_INC(cb, swb_undef_method);
        return false;
    case VM_METHOD_TYPE_NOTIMPLEMENTED:
        GEN_COUNTER_INC(cb, swb_not_implemented_method);
        return false;
    case VM_METHOD_TYPE_OPTIMIZED:
        GEN_COUNTER_INC(cb, swb_optimized_method);
        return false;
    case VM_METHOD_TYPE_MISSING:
        GEN_COUNTER_INC(cb, swb_missing_method);
        return false;
    case VM_METHOD_TYPE_REFINED:
        GEN_COUNTER_INC(cb, swb_refined_method);
        return false;
