...
mod bits {
  ...
  pub const Any: u16 = Allocator | Control | Memory | PatchPoint;
  pub const Control: u16 = ToInterpreter | ToJIT;
  pub const Empty: u16 = 0u16;
  pub const Frame: u16 = Locals | PC | Stack;
  pub const InterruptFlag: u16 = 1u16 << 1;
  pub const Locals: u16 = 1u16 << 2;
  pub const Memory: u16 = Frame | InterruptFlag | Other;
  pub const Other: u16 = 1u16 << 3;
  pub const PC: u16 = 1u16 << 4;
  pub const PatchPoint: u16 = 1u16 << 5;
  pub const Stack: u16 = 1u16 << 6;
  pub const ToInterpreter: u16 = 1u16 << 7;
  pub const ToJIT: u16 = 1u16 << 8;
  pub const AllBitPatterns: [(&str, u16); 14] = [
    ("Any", Any),
    ("Control", Control),
    ("ToJIT", ToJIT),
...
pub mod abstract_heaps {
  ...
  pub const ToInterpreter: AbstractHeap = AbstractHeap::from_bits(bits::ToInterpreter);
  pub const ToJIT: AbstractHeap = AbstractHeap::from_bits(bits::ToJIT);
}
pub mod effects {
  use super::*;
  ...
  pub const ToInterpreter: Effect = Effect::promote(abstract_heaps::ToInterpreter);
  pub const ToJIT: Effect = Effect::promote(abstract_heaps::ToJIT);
}
