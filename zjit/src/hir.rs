...
impl Function {
    ...
    fn validate_definite_assignment(&self) -> Result<(), ValidationError> {
        ...
        let rpo = self.rpo();
        // Begin with every block having every variable defined, except for entries_block and
        // entry blocks, which start with nothing defined.
        let entry_blocks = self.entry_blocks();
        for &block in &rpo {
            if block == self.entries_block || entry_blocks.contains(&block) {
        ...
        while let Some(block) = worklist.pop_front() {
            ...
            for &insn_id in &self.blocks[block.0].insns {
                ...
                match self.find(insn_id) {
                    ...
                    Insn::Entries { ref targets } => {
                        // Entries is the root — unconditionally enqueue all entry blocks
                        for target in targets {
                            worklist.push_back(*target);
                        }
                    }
...
impl<'a> std::fmt::Display for FunctionPrinter<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        for block_id in fun.rpo() {
            // Skip the entries superblock — it's an internal CFG artifact
            if block_id == fun.entries_block { continue; }
            write!(f, "{block_id}(")?;
